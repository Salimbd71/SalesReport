...
export default function App() {
  ...
  const [lastMonthDateHeader, setLastMonthDateHeader] = useState<string>('');
  const [currentSalesHeader, setCurrentSalesHeader] = useState<string>('');
  const [lastMonthSalesHeader, setLastMonthSalesHeader] = useState<string>('');
  ...
  const handleUploadSalesFile = async (file: File) => {
    ...
    try {
      ...
      setCurrentSalesHeader(
        result.salesUptoHeader || formatSalesUptoHeader(result.dateHeader, 'CURRENT MONTH SALES')
      );
  ...
  const handleUploadLastMonthFile = async (file: File) => {
    ...
    try {
      ...
      setLastMonthDateHeader(result.dateHeader || '');
      setLastMonthSalesHeader(
        result.salesUptoHeader || formatSalesUptoHeader(result.dateHeader, 'LAST MONTH SALES')
      );
