...
export const ChemistMasterView: React.FC<ChemistMasterViewProps> = ({
  ...
  return (
    <div id="chemist-master-container" className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
      ...
      {/* Header */}
      <div className="p-5 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center space-x-2">
            ...
            <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100">
              Chemist List Master Sheet ({sheetName})
            </h3>
          </div>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            Reference master list used for VLOOKUP mapping into Sales data
          </p>
        </div>
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search Chemist, Code, FLM, HQ..."
            className="pl-8 pr-3 py-1.5 bg-white dark:bg-slate-800 text-xs border border-slate-300 dark:border-slate-700 rounded-lg text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-1 focus:ring-emerald-500 w-64"
          />
        </div>
      </div>
      ...
      <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/40 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
        ...
        <div className="flex items-center space-x-2">
          ...
          <span className="px-3 py-1 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg font-mono">
