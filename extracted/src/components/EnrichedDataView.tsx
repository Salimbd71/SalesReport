...
export const EnrichedDataView: React.FC<EnrichedDataViewProps> = ({
  ...
  return (
    <div id="enriched-data-container" className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
      ...
      <div className="p-4 sm:p-5 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div>
            <div className="flex items-center space-x-2">
              <div className="w-8 h-8 rounded-lg bg-teal-600/10 text-teal-600 flex items-center justify-center shrink-0">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
              ...
              {isCompareMode && (
                <span className="ml-1 text-indigo-600 dark:text-indigo-400 font-semibold">
                  • {language === 'bn' ? 'শীট ২ এ মাসভিত্তিক তুলনা চালু আছে' : 'Month-on-Month Comparison Active on Sheet 2'}
                </span>
              )}
            </p>
          </div>
          ...
          >
            ...
            <span>{isExportingConsolidated ? t.exportingConsolidated : t.exportConsolidatedExcel}</span>
          </button>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2.5 mt-4 sm:mt-5">
          <button
            ...
            className={`px-3.5 sm:px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center space-x-2 ${
              subView === 'sheet4'
          ...
          >
            ...
            <span>{t.sheet2Tab}</span>
            <span className={`px-2 py-0.5 rounded-full text-[10px] font-mono ${
              subView === 'sheet2' ? 'bg-teal-800 text-teal-100' : 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300'
            }`}>
              {isCompareMode ? (language === 'bn' ? 'তুলনা (চলতি বনাম গত মাস)' : 'Comparison (Current vs Last)') : 'Col K (FLM) & L (HQ)'} • {customerRecords.length.toLocaleString()} {t.rows}
            </span>
          </button>
        </div>
      </div>
      ...
      {subView === 'sheet2' && isCompareMode && comparisonStats && (
        <div className="p-3 sm:p-4 bg-indigo-50/50 dark:bg-indigo-950/20 border-b border-indigo-100 dark:border-indigo-900/40">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3 text-xs">
            
            <div className="p-2.5 sm:p-3 rounded-xl bg-white dark:bg-slate-900 border border-indigo-200 dark:border-indigo-800 shadow-sm">
            ...
            <div className={`p-2.5 sm:p-3 rounded-xl bg-white dark:bg-slate-900 border shadow-sm ${
              ...
              <div className="text-[11px] text-slate-500 dark:text-slate-400 font-medium flex items-center justify-between">
                <span>{language === 'bn' ? 'নেট ঘাটতি / প্রবৃদ্ধি' : 'Net Deficit / Growth'}</span>
                {comparisonStats.netDeficit < 0 ? (
            ...
            <div className="p-2.5 sm:p-3 rounded-xl bg-white dark:bg-slate-900 border border-indigo-200 dark:border-indigo-800 shadow-sm">
              <div className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">{t.deficitChemistCount}</div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2.5 sm:gap-3 text-xs">
          ...
          {subView === 'sheet4' ? (
            <div>
              ...
              >
                <option value="ALL">{t.allBrands} ({uniqueBrands.length})</option>
                {uniqueBrands.map((b) => (
            ...
            <div>
              ...
              >
                <option value="ALL">{language === 'bn' ? 'সকল তুলনা' : 'All Month Comparisons'}</option>
                <option value="DEFICIT_ONLY">📉 {language === 'bn' ? 'ঘাটতি শুধুমাত্র (নেতিবাচক)' : 'Deficit Only (Drop / Negative)'}</option>
                <option value="GROWTH_ONLY">📈 {language === 'bn' ? 'প্রবৃদ্ধি শুধুমাত্র (ইতিবাচক)' : 'Growth Only (Positive)'}</option>
                <option value="LAST_ONLY">⏮ {language === 'bn' ? 'গত মাস শুধুমাত্র' : 'Last Month Only (0 in Current)'}</option>
                <option value="CURRENT_ONLY">⏭ {language === 'bn' ? 'চলতি মাস শুধুমাত্র' : 'Current Month Only (0 in Last)'}</option>
              </select>
            </div>
          ) : null}
          ...
          <div>
            <select
              ...
              className="w-full py-1.5 px-2 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-1 focus:ring-teal-500 text-xs"
            >
              <option value="ALL">{t.allFlms} ({uniqueFlms.length})</option>
              {uniqueFlms.map((f) => (
          ...
          <div>
            <select
              ...
              className="w-full py-1.5 px-2 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-1 focus:ring-teal-500 text-xs"
            >
              <option value="ALL">{t.allMatchStatus}</option>
              <option value="MATCHED">✓ {t.matchedInChemist}</option>
              <option value="UNMATCHED">⚠ {t.unmatchedStatus}</option>
            </select>
          </div>
      ...
      {dateHeader && (
        <div className="bg-amber-50 dark:bg-amber-950/20 border-b border-amber-200 dark:border-amber-800/50 px-6 py-2 flex items-center space-x-2 text-xs text-amber-900 dark:text-amber-200">
          <span className="font-bold">{t.reportDate} (A1):</span>
      ...
      <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/40 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
        <div className="text-slate-500 dark:text-slate-400">
          {language === 'bn' ? (
            <>
              মোট <strong>{activeRecordsCount.toLocaleString()}</strong> সারির মধ্যে <strong>{(currentPage - 1) * pageSize + 1}</strong> থেকে{' '}
              <strong>{Math.min(currentPage * pageSize, activeRecordsCount)}</strong> দেখানো হচ্ছে (সর্বমোট: {subView === 'sheet4' ? records.length.toLocaleString() : customerRecords.length.toLocaleString()})
            </>
          ) : (
            <>
