    ...
    <div id="pivot-table-container" className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
              Structure: <strong>{t.flmHeader} &gt; {t.hqHeader}</strong> | <strong>Columns: BRAND</strong> | <strong>Value: Sum of SALES_VALUE in Lac</strong>
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder={t.searchFlmHqBrand}
            ...
            >
              ...
              <span>{sortBy === 'sales_desc' ? t.sortBySales : t.sortDefault}</span>
            </button>
            ...
            {/* Download Consolidated Sales File Excel */}
            <button
              id="btn-download-pivot-table"
              onClick={onExportConsolidated}
              disabled={isExportingConsolidated || isExportingPivot}
              className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 disabled:bg-emerald-800/60 disabled:cursor-not-allowed text-white rounded-lg text-xs font-semibold flex items-center space-x-1.5 shadow-sm cursor-pointer ml-auto transition-colors"
              title="Download Consolidated Sales File with Pivot Table"
            >
          ...
          <thead className="bg-slate-900 text-white sticky top-0 z-20 shadow-sm">
            <tr>
              ...
              <th className="py-3 px-4 font-bold text-right text-emerald-300 min-w-[150px] bg-slate-950 border-l border-slate-800">
                {formatMode === 'lac' ? `${t.grandTotal} (Lac)` : formatMode === 'currency' ? `${t.grandTotal} (৳)` : t.grandTotal}
              </th>
            </tr>
          </thead>
                  ...
                  <React.Fragment key={group.flm}>
                    ...
                    >
                      ...
                      <td className="py-2.5 px-4 text-blue-800 dark:text-blue-300 text-xs italic border-r border-blue-200 dark:border-blue-900">
                        {language === 'bn' ? 'সকল HQ উপ-মোট' : 'All HQs Subtotal'}
                      </td>
          ...
          <tfoot className="bg-slate-900 text-white font-bold sticky bottom-0 z-20 shadow-lg">
            <tr>
              ...
              <td className="py-3 px-4 text-slate-300 border-r border-slate-800 text-xs">
                {language === 'bn' ? `সকল ${pivotData.flmGroups.length} FLM` : `All ${pivotData.flmGroups.length} FLMs`}
              </td>
              {pivotData.brands.map((brand) => (
                <td
                  key={brand}
      ...
      <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/40 flex flex-wrap items-center justify-between gap-4 text-xs">
        <div className="flex items-center space-x-4 text-slate-600 dark:text-slate-300">
          <span>
            {t.flmManagers}: <strong>{pivotData.flmGroups.length}</strong>
          </span>
          <span>•</span>
          <span>
            {t.activeBrands}: <strong>{pivotData.brands.length}</strong>
          </span>
          <span>•</span>
          <span>
            {t.totalRecords}: <strong>{pivotData.totalRecords.toLocaleString()}</strong>
          </span>
        </div>
        ...
        <div className="flex items-center space-x-3">
          <button
            onClick={onExportConsolidated}
            disabled={isExportingConsolidated || isExportingPivot}
            className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white rounded-lg font-medium shadow-sm transition-all cursor-pointer flex items-center space-x-1.5"
          >
            {isExportingConsolidated ? (
              <Loader2 className="w-3.5 h-3.5 animate-spin text-emerald-200" />
            ) : (
            ...
            <span>{isExportingConsolidated ? t.exporting : t.exportConsolidatedExcel}</span>
          </button>
        </div>
      </div>
    </div>
