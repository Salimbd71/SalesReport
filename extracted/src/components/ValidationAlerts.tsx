    ...
    <div id="validation-section" className="space-y-3 mb-6">
        ...
        >
          <div className="flex items-start justify-between gap-2">
              ...
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  <h4 className="font-semibold text-xs sm:text-sm">
                    {language === 'bn' ? 'চলতি মাসের সেলস ফাইল ভ্যালিডেশন: ' : 'Sales Data File Validation: '}
                    {hasSalesIssues 
                      ? (language === 'bn' ? 'ফরম্যাট সতর্কতা' : 'Formatting Notice / Warnings') 
                      : (language === 'bn' ? 'সঠিক ফরম্যাট যাচাইকৃত' : 'Format Verified Perfectly')}
                  </h4>
                  <span className="text-[11px] px-2 py-0.5 rounded-full font-mono bg-white/60 dark:bg-slate-800/60 border border-current/20">
                    {language === 'bn' ? 'শীট: ' : 'Sheet: '}{salesValidation.sheetName} ({salesValidation.rowCount} {t.rows})
                  </span>
                </div>

                <p className="text-xs mt-1 text-slate-600 dark:text-slate-300">
                  {hasSalesIssues
                    ? (language === 'bn' ? 'কিছু কলাম বা শীট কনফিগারেশন স্ট্যান্ডার্ড ফরম্যাট থেকে আলাদা।' : 'Some columns or sheet configuration differed from standard format. Check details below.')
                    : (language === 'bn' ? 'টার্গেট শীট (HQ-Customer-Product Sales / Sheet 4), সেল A1 তারিখ এবং সকল ১৫টি সেলস কলাম সফলভাবে শনাক্ত হয়েছে।' : 'Target sheet (HQ-Customer-Product Sales / Sheet 4), cell A1 Date header, and all 15 sales columns detected.')}
                </p>

                {/* Sheet Selector if multi-sheet */}
                {salesValidation.availableSheets.length > 1 && onSelectSalesSheet && (
                  <div className="mt-2 flex items-center space-x-2 text-xs">
                    <span className="font-medium">{language === 'bn' ? 'সক্রিয় শীট:' : 'Active Sheet:'}</span>
                    <select
            ...
            <button
              ...
              className="text-xs flex items-center space-x-1 opacity-80 hover:opacity-100 cursor-pointer shrink-0"
            >
              <span>{expandedSales ? (language === 'bn' ? 'লুকান' : 'Hide') : (language === 'bn' ? 'কলাম দেখুন' : 'Check Columns')}</span>
          ...
          {expandedSales && (
            <div className="mt-3 pt-3 border-t border-current/15 text-xs">
              <div className="font-medium mb-1">{language === 'bn' ? 'সেলস কলাম সমূহের স্ট্যাটাস:' : 'Expected Sales Columns Status:'}</div>
      ...
      {chemistValidation && (
        ...
        >
          <div className="flex items-start justify-between gap-2">
            <div className="flex items-start space-x-3">
              ...
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  <h4 className="font-semibold text-xs sm:text-sm">
                    {language === 'bn' ? 'কেমিস্ট লিস্ট ফাইল ভ্যালিডেশন: ' : 'Chemist List File Validation: '}
                    {hasChemistIssues ? (language === 'bn' ? 'ফরম্যাট সতর্কতা' : 'Formatting Notice') : (language === 'bn' ? 'সঠিক ফরম্যাট যাচাইকৃত' : 'Format Verified Perfectly')}
                  </h4>
                  <span className="text-[11px] px-2 py-0.5 rounded-full font-mono bg-white/60 dark:bg-slate-800/60 border border-current/20">
                    {language === 'bn' ? 'শীট: ' : 'Sheet: '}{chemistValidation.sheetName} ({chemistValidation.rowCount} {language === 'bn' ? 'জন কেমিস্ট' : 'chemists'})
                  </span>
                </div>

                <p className="text-xs mt-1 text-slate-600 dark:text-slate-300">
                  {hasChemistIssues
                    ? (language === 'bn' ? 'প্রত্যাশিত শীট DHK-MYN-KH বা টার্গেট কলাম ম্যাপিং প্রয়োজন।' : 'Expected sheet DHK-MYN-KH or target columns (Cust code, Col I: FSM-(NEW)2026-27, Col K: AZURA HQ) needed mapping.')
                    : (language === 'bn' ? 'টার্গেট শীট (DHK-MYN-KH), Cust code, কলাম I (FLM), এবং কলাম K (HQ) সফলভাবে ম্যাপ হয়েছে।' : 'Target sheet (DHK-MYN-KH), Cust code, Column I (FLM), and Column K (HQ) mapped successfully.')}
                </p>
            ...
            <button
              ...
              className="text-xs flex items-center space-x-1 opacity-80 hover:opacity-100 cursor-pointer shrink-0"
            >
              <span>{expandedChemist ? (language === 'bn' ? 'লুকান' : 'Hide') : (language === 'bn' ? 'কলাম দেখুন' : 'Check Columns')}</span>
          ...
          {expandedChemist && (
            <div className="mt-3 pt-3 border-t border-current/15 text-xs">
              <div className="font-medium mb-1">{language === 'bn' ? 'ম্যাপ করা মূল কেমিস্ট কলামসমূহ:' : 'Key Chemist Columns Mapped:'}</div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                <div className="p-2 rounded bg-emerald-100/60 dark:bg-emerald-950/40 border border-emerald-300">
                  <div className="font-bold text-emerald-800 dark:text-emerald-300">{language === 'bn' ? 'লুকআপ কী:' : 'Lookup Key:'}</div>
                ...
                <div className="p-2 rounded bg-teal-100/60 dark:bg-teal-950/40 border border-teal-300">
                  <div className="font-bold text-teal-800 dark:text-teal-300">{language === 'bn' ? 'FLM কলাম:' : 'FLM Column:'}</div>
                ...
                <div className="p-2 rounded bg-blue-100/60 dark:bg-blue-950/40 border border-blue-300">
                  <div className="font-bold text-blue-800 dark:text-blue-300">{language === 'bn' ? 'HQ কলাম:' : 'HQ Column:'}</div>
      ...
      {isCompareMode && lastMonthValidation && (
        ...
        >
          <div className="flex items-start justify-between gap-2">
            <div className="flex items-start space-x-3">
              ...
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  <h4 className="font-semibold text-xs sm:text-sm">
                    {language === 'bn' ? 'গত মাসের সেলস ফাইল (শীট ২) ভ্যালিডেশন: ' : 'Last Month Sales File (Sheet 2) Validation: '}
                    {hasLastMonthIssues ? (language === 'bn' ? 'ফরম্যাট সতর্কতা' : 'Formatting Notice / Missing Columns') : (language === 'bn' ? 'সঠিক ফরম্যাট যাচাইকৃত' : 'Format Verified Perfectly')}
                  </h4>
                  <span className="text-[11px] px-2 py-0.5 rounded-full font-mono bg-white/60 dark:bg-slate-800/60 border border-current/20">
                    {language === 'bn' ? 'ডিফল্ট শীট ২: ' : 'Default Sheet 2: '}{lastMonthValidation.sheetName} ({lastMonthValidation.rowCount} {t.rows})
                  </span>
                </div>

                <p className="text-xs mt-1 text-slate-600 dark:text-slate-300">
                  {hasLastMonthIssues
                    ? (language === 'bn' ? 'HQ-Customer Sales ফরম্যাটের প্রয়োজনীয় কিছু কলাম অনুপস্থিত।' : 'Some required columns for HQ-Customer Sales format are missing or differ. Check details below.')
                    : (language === 'bn' ? '২ নম্বর শীট (HQ-Customer Sales) ডিফল্ট হিসেবে সেট করা এবং সকল ১০টি প্রয়োজনীয় কলাম যাচাইকৃত।' : '2 number sheet (HQ-Customer Sales) set as default. All 10 required customer sales columns verified.')}
                </p>
            ...
            <button
              ...
              className="text-xs flex items-center space-x-1 opacity-80 hover:opacity-100 cursor-pointer shrink-0"
            >
              <span>{expandedLastMonth ? (language === 'bn' ? 'লুকান' : 'Hide') : (language === 'bn' ? 'কলাম দেখুন' : 'Check Columns')}</span>
          ...
          {expandedLastMonth && (
            <div className="mt-3 pt-3 border-t border-current/15 text-xs">
              <div className="font-medium mb-1">{language === 'bn' ? 'শীট ২ (HQ-Customer Sales) এর ১০টি প্রয়োজনীয় কলাম:' : 'Sheet 2 (HQ-Customer Sales) 10 Required Columns Status:'}</div>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-1.5">
                {[
                  'HQ_CODE',
                  'HQ_NAME',
      ...
      {unmatchedCount > 0 && (
        ...
        >
          ...
          {unmatchedCodes.length > 0 && (
            ...
            >
              {showUnmatchedList ? (language === 'bn' ? 'লুকান' : 'Hide') : (language === 'bn' ? `${unmatchedCodes.length}টি আনম্যাচড কোড দেখুন` : `View ${unmatchedCodes.length} Unmatched Codes`)}
            </button>
          )}
        </div>
      )}
      ...
      {showUnmatchedList && unmatchedCodes.length > 0 && (
        <div className="p-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg text-xs">
          <div className="font-semibold text-slate-700 dark:text-slate-300 mb-1.5">
            {language === 'bn' ? 'আনম্যাচড গ্রাহক কোড (CUST_CODE):' : 'Unmatched Customer Codes (CUST_CODE):'}
          </div>
