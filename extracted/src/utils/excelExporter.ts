...
import { formatSalesUptoHeader, parseLastDateInfo } from './excelParser';
...
function buildEnrichedSalesSheet(
  ...
  dateHeader: string,
  currentSalesColHeader = 'SALES_VALUE'
) {
  // Row 1: Executive Title
  const titleText = dateHeader
    ? `HQ-CUSTOMER-PRODUCT SALES REPORT (${dateHeader})`
    : 'HQ-CUSTOMER-PRODUCT SALES REPORT';
  ...
  const subRow = worksheet.addRow([
    `Product-level Enriched Sales Data with VLOOKUP Mappings (FLM & HQ) | Total Records: ${salesRecords.length.toLocaleString()}`,
  ]);
  subRow.height = 18;
  ...
  // Highlight sales header with teal and newly added P (FLM) and Q (HQ)
  headerRow.getCell(15).fill = {
    type: 'pattern',
    pattern: 'solid',
  ...
  headerRow.getCell(16).fill = {
    type: 'pattern',
    pattern: 'solid',
    fgColor: { argb: '0F766E' }, // Teal Accent
  };
  headerRow.getCell(17).fill = {
    type: 'pattern',
    pattern: 'solid',
    fgColor: { argb: '115E59' }, // Darker Teal
  };
  ...
  salesRecords.forEach((rec, idx) => {
    const expQty = rec.EXP_QTY_BOX || 0;
    const expVal = rec.EXP_VALUE || 0;
    const salesQty = rec.SALES_QTY_BOX || 0;
    ...
    // Format numbers and align in Middle (Center)
    ...
    // Cell borders and default vertical alignment
    row.eachCell((cell, colNum) => {
      if (colNum < 12 || colNum > 15) {
        cell.alignment = { vertical: 'middle', horizontal: colNum >= 16 ? 'center' : 'left' };
      }
      cell.border = {
...
function buildEnrichedHqCustomerSheet(
  ...
  dateHeader: string,
  isCompareMode = false,
  currentSalesColHeader = 'CURRENT MONTH SALES',
  lastSalesColHeader = 'LAST MONTH SALES'
) {
  if (isCompareMode) {
    // Comparison Mode
    // Row 1: Executive Title
    const titleRow = worksheet.addRow([
      dateHeader
        ? `HQ-CUSTOMER SALES REPORT (${dateHeader}) - MONTH-ON-MONTH COMPARISON`
        : 'HQ-CUSTOMER SALES REPORT (MONTH-ON-MONTH COMPARISON)',
    ]);
    titleRow.height = 28;
    titleRow.getCell(1).font = { bold: true, size: 14, color: { argb: '1E3A8A' } };
    ...
    // Row 2: Subtitle
    const subRow = worksheet.addRow([
      `Customer-level Sales with VLOOKUP Mappings (FLM & HQ) | Formula: DEFICIT = ${currentSalesColHeader} - ${lastSalesColHeader} | Total Customers: ${customerRecords.length.toLocaleString()}`,
    ]);
    subRow.height = 18;
    subRow.getCell(1).font = { italic: true, size: 10, color: { argb: '475569' } };
    ...
    customerRecords.forEach((rec, idx) => {
      ...
      // Format numbers & align sales values in Middle (Center)
      ...
      // Deficit font color (- value red text, + value green text)
      if (deficitVal < 0) {
        row.getCell(12).font = { bold: true, color: { argb: 'DC2626' } };
      } else if (deficitVal > 0) {
        row.getCell(12).font = { bold: true, color: { argb: '059669' } };
      }
      ...
      row.eachCell((cell, colNum) => {
        if (colNum < 7 || colNum > 12) {
          cell.alignment = { vertical: 'middle', horizontal: colNum >= 13 ? 'center' : 'left' };
        }
        cell.border = {
          top: { style: 'thin', color: { argb: 'F3F4F6' } },
          bottom: { style: 'thin', color: { argb: 'E5E7EB' } },
    ...
    worksheet.columns.forEach((col, idx) => {
      let maxLen = headers[idx] ? headers[idx].length : 12;
      if (idx === 4) maxLen = 28; // MHL_CUST_NAME
      if (idx === 9 || idx === 10 || idx === 11) maxLen = 22; // Sales values & Deficit
      if (idx === 12 || idx === 13) maxLen = 22; // FLM & HQ
      col.width = Math.max(maxLen + 4, 12);
    });
  } else {
    // Standard Mode (Original columns)
    // Row 1: Executive Title
    const titleRow = worksheet.addRow([
      dateHeader ? `HQ-CUSTOMER SALES REPORT (${dateHeader})` : 'HQ-CUSTOMER SALES REPORT',
    ]);
    titleRow.height = 28;
    titleRow.getCell(1).font = { bold: true, size: 14, color: { argb: '1E3A8A' } };
    ...
    // Row 2: Subtitle
    const subRow = worksheet.addRow([
      `Customer-level Enriched Sales Data with VLOOKUP Mappings (FLM & HQ) | Total Customers: ${customerRecords.length.toLocaleString()}`,
    ]);
    subRow.height = 18;
    subRow.getCell(1).font = { italic: true, size: 10, color: { argb: '475569' } };
    ...
    // Highlight newly added K (FLM) and L (HQ) with teal accent
    ...
    customerRecords.forEach((rec, idx) => {
      ...
      // Format numbers & align in Middle (Center)
      row.getCell(7).numFmt = '#,##0';
      row.getCell(7).alignment = { vertical: 'middle', horizontal: 'center' };
      ...
      row.eachCell((cell, colNum) => {
        if (colNum < 7 || colNum > 10) {
          cell.alignment = { vertical: 'middle', horizontal: colNum >= 11 ? 'center' : 'left' };
        }
        cell.border = {
          top: { style: 'thin', color: { argb: 'F3F4F6' } },
          bottom: { style: 'thin', color: { argb: 'E5E7EB' } },
    ...
    worksheet.columns.forEach((col, idx) => {
      let maxLen = headers[idx] ? headers[idx].length : 12;
      if (idx === 4) maxLen = 28; // MHL_CUST_NAME
      if (idx === 9) maxLen = 22; // Sales Value
...
export function buildCurrentVsLastMonthSheet(
  ...
  // Subtitle Row
  const subRow = worksheet.addRow([
    `Month-on-Month Performance Analysis | Value in Lac (10,000 = 0.10 | 100,000 = 1.00) | Formula: DEFICIT = ${currentSalesColHeader} - ${lastSalesColHeader} | Structure: FLM (First Line Manager) > HQ (Headquarter)`,
  ]);
  subRow.height = 20;
  ...
  // Table Headers (Row 4): Matches the requested dynamic headers
...
function buildHqWiseBrandSheet(
  ...
  const subText = `Values in Lac (10,000 = 0.10 | 100,000 = 1.00) | Sorted by HQ | All HQ Total: ${toLac(
    ...
    isCompare ? ` | Formula: DEFICIT = ${currentSalesColHeader} - ${lastSalesColHeader}` : ''
  }`;
  ...
  const headers = isCompare
    ? ['FLM', 'HQ', 'BRAND', currentSalesColHeader, lastSalesColHeader, 'DEFICIT']
    : ['FLM', 'HQ', 'BRAND', currentSalesColHeader];
    ...
    if (item.isHqTotal) {
      ...
      row.eachCell((cell, colNum) => {
        ...
        if (colNum === 1) {
          ...
          // CURRENT MONTH SALES (Center)
          cell.font = { bold: true, size: 10, color: { argb: '0F172A' } };
          cell.numFmt = '#,##0.00';
          cell.alignment = { vertical: 'middle', horizontal: 'center' };
        } else if (colNum === 5) {
          // LAST MONTH SALES (Center)
  ...
  worksheet.getColumn(3).width = 24; // BRAND
  worksheet.getColumn(4).width = 24; // CURRENT MONTH SALES
  if (isCompare) {
    worksheet.getColumn(5).width = 24; // LAST MONTH SALES
 ...
 * "Consolidated Sales File Upto-31 Aug.xlsx" or "Consolidated Sales File Upto-5 Sep.xlsx"
 */
export function getConsolidatedFileName(dateHeader: string): string {
  ...
  const info = parseLastDateInfo(dateHeader);
  if (info) {
    return `Consolidated Sales File Upto-${info.day} ${info.formattedMonth}.xlsx`;
  }
...
export async function exportConsolidatedFile(
  ...
  isCompareMode = false,
  hqBrandData?: HqWiseBrandData,
  lastMonthDateHeader?: string,
  currentSalesHeader?: string,
  lastMonthSalesHeader?: string,
  ...
  // Dynamic Column Headlines for Sheet 2, 3, 4:
  // e.g., SALES-UPTO-5-SEP and SALES-UPTO-5-AUG
  const curColHeader =
    currentSalesHeader || formatSalesUptoHeader(dateHeader, 'CURRENT MONTH SALES');
  const lastColHeader =
    lastMonthSalesHeader || formatSalesUptoHeader(lastMonthDateHeader, 'LAST MONTH SALES');
  ...
  const resolvedHqBrandData =
    (hqBrandData && hqBrandData.items.length > 0)
      ? hqBrandData
      : generateHqWiseBrandData(salesRecords, lastMonthSalesRecords, isCompareMode);

  if (resolvedHqBrandData && resolvedHqBrandData.items.length > 0) {
    const hqBrandSheet = workbook.addWorksheet('HQ Wise Brand', {
      views: [{ state: 'frozen', xSplit: 2, ySplit: 4 }],
    });
    buildHqWiseBrandSheet(
      hqBrandSheet,
      resolvedHqBrandData,
      dateHeader,
      curColHeader,
  ...
  buildEnrichedSalesSheet(salesSheet, salesRecords, dateHeader, curColHeader);
